...
class DiffusionWrapper(nn.Module):
    ...
    def forward(self, x, t, cond):
        return self.diffusion_model(x, t, **cond)
...
class DDPM(pl.LightningModule):
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        target = (x_start - grad).detach()
        # d(loss)/d(latents) = latents - target = latents - (latents - grad) = grad
        loss = 0.5 * F.mse_loss(x_start, target, reduction="sum") / x_start.shape[0]
    ...
    def get_input(self, batch, k):
        ...
        if len(x.shape) == 5:
            ...
            if x.shape[0] == 1:
                x = x[:1,...]
        # x = rearrange(x, 'b h w c -> b c h w')  # x is from 3DGS
        x = x.to(memory_format=torch.contiguous_format).float()
        poses = batch['poses'].squeeze(0)
        poses = poses[:, [0, 2, 1, 3]]
        poses[:, 1] *= -1
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        # camera = get_camera(4, elevation=15, azimuth_start=90, azimuth_span=360)
        # self.camera = camera.repeat(4//4,1).to(self.device)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x, self.camera = super().get_input(batch, k)
    ...
    def apply_model(self, x_noisy, t, cond):
        if isinstance(cond, dict):
            ...
            if self.model.conditioning_key == 'concat':
                key = 'c_concat'
                cond = {key: cond}
            elif self.model.conditioning_key == 'mvdream':
                if cond[0] is not None:
                    if x_noisy.shape[0] == 1:
                        cond = {'context': cond[0][0].repeat(x_noisy.shape[0],1,1)}
                        return self.model(x_noisy, t, cond)
                    else:
                        cond = {'context': cond[0][0].repeat(x_noisy.shape[0],1,1)}
                        cond['camera'] = self.camera
                        cond['num_frames'] = 4
                    return self.model(x_noisy, t, cond)
                # cond = {key: cond}
            else:
...
#         self.conditioning_key = conditioning_key
#         assert self.conditioning_key in [None, 'concat', 'crossattn', 'hybrid', 'adm', 'mvdream']

#     def forward(self, x, t, c_concat = None, c_crossattn: list = None):
...
#         elif self.conditioning_key == 'mvdream':
#             # cc = c_crossattn[0]
#             out = self.diffusion_model(x, t, **c_concat)
#         else:
#             raise NotImplementedError()
