    ...
    def forward(
        ...
        for placeholder_string, placeholder_token in self.string_to_token_dict.items():
            ...
            if self.max_vectors_per_token == 1: # If there's only one vector per token, we can do a simple replacement
                ...
                # print(embedded_text[placeholder_idx])
                embedded_text[placeholder_idx] = placeholder_embedding

                # print(embedded_text[placeholder_idx])
                # print(embedded_text[placeholder_idx].shape)
                ...
                print("placeholder_rows, placeholder_cols ", placeholder_rows,  placeholder_cols)
                if placeholder_rows.nelement() == 0:
                    continue

                sorted_cols, sort_idx = torch.sort(placeholder_cols, descending=False)
                sorted_rows = placeholder_rows[sort_idx]
                # print("before change text:",embedded_text)
                for idx in range(len(sorted_rows)):
                    ...
                    original_embed_row = embedded_text[row].clone()
                    embedded_text[row] = original_embed_row * (1 - self.blend_ratio) + new_embed_row * self.blend_ratio
                    tokenized_text[row] = new_token_row
                    print("fuison")
                print("after change text:",embedded_text)
